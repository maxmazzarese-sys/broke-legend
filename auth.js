...
function paintProfile(){const s=session();profileBtn.textContent=s?s.user.slice(0,1).toUpperCase():"?";const nameEl=document.getElementById("menuName");if(nameEl)nameEl.textContent=s?s.user:"—";if(s){gate.classList.add("hidden");localStorage.setItem("bl_name",s.user);}else{gate.classList.remove("hidden");menu.classList.remove("show");}}
...
form.onsubmit=async e=>{
  e.preventDefault();
  const user=normUser(userEl.value), pass=passEl.value;
  errEl.textContent="";
  if(!validUser(user))return errEl.textContent="Username: 3–16 letters, numbers, _";
  ...
  if(mode==="signup"){
    ...
    map[user]={user,salt:sl,hash,at:Date.now()};
    saveAcc(map);
    setSession({user});
    return;
  }
