...
let shoe=[], player=[], dealer=[], bet=0, inHand=false, hide=true, dealing=false;
const bjPick={val:5};
paintChipBar(document.getElementById("bjChips"),bjPick);
document.getElementById("bjSpot").onclick=()=>{if(inHand||dealing)return;dropChip(()=>bet,n=>{bet=n;totals();},bjPick.val,()=>inHand||dealing);};
document.getElementById("bjClear").onclick=()=>{if(inHand||dealing)return;pullChips(()=>bet,n=>{bet=n;totals();},()=>inHand||dealing);say("Chips back.");};
...
function lock(on){dealing=on;["dealBtn","hitBtn","standBtn","doubleBtn","bjClear"].forEach(id=>{const b=document.getElementById(id);if(b)b.disabled=on;});}
async function start(){if(dealing)return;if(bet<1){say("Drop a chip on the circle first.");return;}player=[];dealer=[];hide=true;clearHands();totals();phase(true);lock(true);say("Dealing...");player.push(draw());seat("p");await sleep(170);dealer.push(draw());seat("d");await sleep(170);player.push(draw());seat("p");await sleep(170);dealer.push(draw());seat("d",true);totals();lock(false);if(isBJ(player)||isBJ(dealer))finish();else say("Hit, stand, or double.");}
...
async function finish(){if(dealing&&inHand===false)return;const p=val(player);if(p>21){hide=false;setStreak(0);say(ROASTS[Math.floor(Math.random()*ROASTS.length)]+" -"+bet);bet=0;phase(false);totals();return;}lock(true);await dealerDraw();const d=val(dealer);let pay=0,text="";if(isBJ(player)&&!isBJ(dealer)){pay=bet+Math.floor(bet*1.5);text="Blackjack. +"+Math.floor(bet*1.5);}else if(d>21||p>d){pay=bet*2;text=WINS[Math.floor(Math.random()*WINS.length)]+" +"+bet;}else if(p===d){pay=bet;text="Push. Chips back.";}else text=ROASTS[Math.floor(Math.random()*ROASTS.length)];if(pay>bet)setStreak(streak()+1);else if(pay===0)setStreak(0);if(pay)setBank(bank()+pay);say(text+(streak()>1?"  Hot x"+streak():""));bet=0;phase(false);totals();lock(false);}
...
function uniq(list){const map={};(list||[]).forEach(p=>{const k=keyName(p.name);if(!k)return;const score=Number(p.score||0), amt=Number(p.amt||0);if(!map[k]||score>map[k].score||amt>map[k].amt)map[k]={name:p.name,score:Math.max(score,map[k]&&map[k].score||0),amt:Math.max(amt,map[k]&&map[k].amt||0)};});return Object.values(map);}
...
async function loadPublicBoard(){try{const res=await fetch(BOARD+"?t="+Date.now(),{cache:"no-store"});if(!res.ok)return;const data=await res.json();if(Array.isArray(data.donors))localStorage.setItem("bl_public_donors",JSON.stringify(uniq(data.donors)));}catch(e){}}
...
document.getElementById("doubleBtn").onclick=async()=>{if(!inHand||dealing||player.length!==2)return toast("Double on first two cards only");if(bank()<bet)return toast("Not enough chips");setBank(bank()-bet);bet*=2;totals();player.push(draw());seat("p");await finish();};
...
shoeNew();setBank(bank());setStreak(streak());render();say("Pick a chip, drop it, then deal.");
