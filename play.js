...
function finish(){const p=val(player);hide=false;if(p>21){document.getElementById("msg").textContent="Bust. Dealer takes $"+bet;phase(false);render();if(bank()<1)setTimeout(toggle,900);return;}const should=Math.random()<0.9;dealerPlay(should);const d=val(dealer);let pay=0,text="";if(isBJ(player)&&!isBJ(dealer)){pay=bet+bet*1.5;text="Blackjack. +$"+(bet*1.5);}else if(should||(p<=21&&(d>21||p>d))){pay=bet*2;text=d>21?"Dealer busts. +$"+bet:"You win. +$"+bet;}else if(p===d){pay=bet;text="Push. Bet returned.";}else text="Dealer wins.";if(pay)setBank(bank()+pay);document.getElementById("msg").textContent=text;phase(false);render();if(bank()<1)setTimeout(toggle,1100);}
