...
function donorList(){return [];}
function playerList(){let list=[];try{list=JSON.parse(localStorage.getItem(PLAYERS)||"[]");}catch(e){list=[];}const me=localStorage.getItem(NAME)||"You";const best=Number(localStorage.getItem(BEST)||0);list=list.filter(p=>p.name!==me);if(best>0)list.push({name:me,score:best,hot:streak()});list.sort((a,b)=>b.score-a.score);return list.slice(0,10);}
function savePlayerName(n){n=(n||"").trim().slice(0,18);if(!n)return toast("Add a table name");localStorage.setItem(NAME,n);localStorage.setItem(PLAYERS,JSON.stringify(playerList()));toast("Name locked in");paintBoard();}
function paintBoard(){const donors=document.getElementById("boardDonors"), players=document.getElementById("boardPlayers");if(!donors||!players)return;const tips=donorList();donors.innerHTML=tips.length?tips.map((d,i)=>'<div class="row"><b>'+(i+1)+". "+d.name+"</b><span>$"+d.amt+"</span></div>").join("")+'<p class="note">Tips from Donate.</p>':'<div class="empty">No donators yet. First chip takes the top seat.</div><button class="cta" id="boardDonate" type="button" style="margin-top:12px">Donate</button>';const rows=playerList();const name=localStorage.getItem(NAME)||"";players.innerHTML='<div class="namebox"><input id="tableName" maxlength="18" placeholder="Your table name" value="'+name.replace(/"/g,"")+'"><button type="button" id="saveName">Save</button></div>'+(rows.length?rows.map((p,i)=>'<div class="row"><b>'+(i+1)+". "+p.name+"</b><span>"+p.score+" chips</span></div>").join(""):'<div class="empty">Play a hand, then save your name.</div>')+'<p class="note">Play chips only. Scores stay on this phone for now.</p>';const bd=document.getElementById("boardDonate");if(bd)bd.onclick=()=>show("donate");const sn=document.getElementById("saveName");if(sn)sn.onclick=()=>savePlayerName(document.getElementById("tableName").value);}
