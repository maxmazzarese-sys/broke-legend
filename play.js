...
const BOARD="board.json";
function keyName(s){return String(s||"").trim().toLowerCase();}
function meName(){try{const s=JSON.parse(localStorage.getItem("bl_session")||"null");if(s&&s.user)return s.user;}catch(e){}return localStorage.getItem(NAME)||"";}
function uniq(list){const map={};(list||[]).forEach(p=>{const k=keyName(p.name);if(!k)return;const score=Number(p.score||0), amt=Number(p.amt||0);const cur=map[k];if(!cur||score>cur.score||amt>cur.amt)map[k]={name:p.name,score:Math.max(score,cur?cur.score:0),amt:Math.max(amt,cur?cur.amt:0)};});return Object.values(map);}
...
function playerList(){let list=[];try{list=JSON.parse(localStorage.getItem(PLAYERS)||"[]");}catch(e){list=[];}const me=meName(), best=Number(localStorage.getItem(BEST)||0);if(me&&best>0)list=list.concat([{name:me,score:best}]);list=uniq(list).sort((a,b)=>b.score-a.score);localStorage.setItem(PLAYERS,JSON.stringify(list.slice(0,20)));return list.slice(0,20);}
async function loadPublicBoard(){try{const res=await fetch(BOARD+"?t="+Date.now(),{cache:"no-store"});if(!res.ok)return;const data=await res.json();if(Array.isArray(data.players))localStorage.setItem(PLAYERS,JSON.stringify(uniq((JSON.parse(localStorage.getItem(PLAYERS)||"[]")).concat(data.players))));if(Array.isArray(data.donors))localStorage.setItem("bl_public_donors",JSON.stringify(uniq(data.donors)));}catch(e){}}
function paintBoard(){const donors=document.getElementById("boardDonors"), players=document.getElementById("boardPlayers");if(!donors||!players)return;const tips=donorList();donors.innerHTML=tips.length?tips.map((d,i)=>'<div class="row"><b>'+(i+1)+". "+d.name+"</b><span>$"+d.amt+"</span></div>").join("")+'<p class="note">Public tip board. One seat per name.</p>':'<div class="empty">No donators yet. First chip takes the top seat.</div><button class="cta" id="boardDonate" type="button" style="margin-top:12px">Donate</button>';const rows=playerList();const me=meName();players.innerHTML=(me?'<p class="note">Signed in as <b style="color:#f6f1e4">'+me+'</b>. One row only.</p>':'<p class="note">Log in to take a seat.</p>')+(rows.length?rows.map((p,i)=>'<div class="row"><b>'+(i+1)+". "+p.name+"</b><span>"+p.score+" chips</span></div>").join(""):'<div class="empty">Play a hand to land on the board.</div>')+'<p class="note">One username, one seat. max / Max / Maxi count as the same name.</p>';const bd=document.getElementById("boardDonate");if(bd)bd.onclick=()=>show("donate");}
loadPublicBoard().then(paintBoard);
document.querySelectorAll("[data-board]").forEach(btn=>btn.onclick=()=>{document.querySelectorAll("[data-board]").forEach(b=>b.classList.toggle("on",b===btn));document.getElementById("boardDonors").style.display=btn.dataset.board==="donators"?"block":"none";document.getElementById("boardPlayers").style.display=btn.dataset.board==="players"?"block":"none";});
...
document.getElementById("betUp").onclick=()=>{if(inHand)return;bet=Math.min(50,bank(),bet+5);totals();};
