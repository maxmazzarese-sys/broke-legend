...
function rainCards(){
  ...
  function spawn(i){
    ...
    bits.push({
      el, x:left?14:W-52, y:H-58,
      vx:(left?1:-1)*Math.cos(ang)*speed*(0.85+Math.random()*0.4),
      vy:-Math.sin(ang)*speed*(1.05+Math.random()*0.35),
      g:0.16+Math.random()*0.08,
      rot:Math.random()*40-20,
      spin:(left?1:-1)*(2.4+Math.random()*4.2),
      flip:Math.random()*180,
      flipV:8+Math.random()*14,
      life:0, max:2600+Math.random()*1400
    });
  }
  let n=0;
  const emitter=setInterval(()=>{
    spawn(n++); spawn(n++);
    if(n>=80)clearInterval(emitter);
  },42);
  ...
  function tick(now){
    ...
    for(let i=bits.length-1;i>=0;i--){
      ...
      c.rot+=c.spin*dt; c.flip+=c.flipV*dt; c.life+=16.67*dt;
