...
async function loadPublicBoard(){try{const res=await fetch("board.json?t="+Date.now(),{cache:"no-store"});if(!res.ok)return;const data=await res.json();if(Array.isArray(data.donors)){const remote=uniq(data.donors);const local=donorList();const merged=uniq(remote.concat(local));saveDonors(merged);}}catch(e){}}
function paintBoard(){const donors=document.getElementById("boardDonors");if(!donors)return;const tips=donorList();donors.innerHTML=tips.length?tips.map((d,i)=>'<div class="row"><b>'+(i===0?"\u265B ":"")+(i+1)+". "+d.name+'</b><span>$'+d.amt+"</span></div>").join("")+'<p class="note">One seat per name. Board refreshes after each tip.</p>':'<div class="empty">Nobody yet. The crown is still on the table.</div>';paintMeter();}
function currentUser(){try{const s=JSON.parse(localStorage.getItem("bl_session")||"null");if(s&&s.user)return s.user;}catch(e){}return localStorage.getItem("bl_name")||"";}
function recordTip(amt){
  ...
  paintBoard(); paintMeter(); show("leaderboard");
}
...
function rainCards(){
  ...
  function spawn(i){const s=suits[i%4], left=i%2===0; const el=document.createElement("div"); el.className="flycard"+((s==="\u2665"||s==="\u2666")?" red":""); el.innerHTML="<b>"+ranks[i%13]+"</b><i>"+s+"</i><span>"+s+"</span>"; document.body.appendChild(el); const sweep=0.35+0.65*Math.abs(Math.sin((performance.now()-start)/420)); const ang=(22+sweep*58)*Math.PI/180; const speed=7.2+Math.random()*5.4; bits.push({el,x:left?14:W-52,y:H-58,vx:(left?1:-1)*Math.cos(ang)*speed*(0.85+Math.random()*0.4),vy:-Math.sin(ang)*speed*(1.05+Math.random()*0.35),g:0.16+Math.random()*0.08,rot:Math.random()*40-20,spin:(left?1:-1)*(2.4+Math.random()*4.2),flip:Math.random()*180,flipV:8+Math.random()*14,life:0,max:2600+Math.random()*1400});}
  let n=0; const emitter=setInterval(()=>{spawn(n++);spawn(n++);if(n>=80)clearInterval(emitter);},42);
  let last=performance.now(); function tick(now){const dt=Math.min(32,now-last)/16.67;last=now;for(let i=bits.length-1;i>=0;i--){const c=bits[i];c.vy+=c.g*dt;c.x+=c.vx*dt;c.y+=c.vy*dt;c.rot+=c.spin*dt;c.flip+=c.flipV*dt;c.life+=16.67*dt;const fade=c.life>c.max-500?Math.max(0,1-(c.life-(c.max-500))/500):1;c.el.style.transform="translate("+c.x+"px,"+c.y+"px) rotate("+c.rot+"deg) rotateY("+c.flip+"deg)";c.el.style.opacity=String(fade);if(c.life>c.max||c.y>H+80){c.el.remove();bits.splice(i,1);}}if(bits.length||n<80)requestAnimationFrame(tick);}requestAnimationFrame(tick);
...
paintMeter();
if(/[?&]tipped=1/.test(location.search)){
  const q=new URLSearchParams(location.search);
  const pending=Number(q.get("amt")||sessionStorage.getItem("bl_pending_tip")||0);
  show("donate");
  setTimeout(rainCards,200);
  setTimeout(()=>{recordTip(pending);sessionStorage.removeItem("bl_pending_tip");loadPublicBoard().then(paintBoard);},1000);
  history.replaceState({}, "", location.pathname+"#leaderboard");
}
(function(){
  ...
  const amts={c1:1,c5:5,c10:10,c25:25,c50:50,c100:100,c1000:1000};
  document.querySelectorAll("a.chip").forEach(a=>{
    Object.keys(byClass).forEach(k=>{if(a.classList.contains(k)){a.href=byClass[k]; a.addEventListener("click",()=>sessionStorage.setItem("bl_pending_tip",String(amts[k])));}});
  });
})();
